...
CHESS_TYPE_NUM = 10
SIX = CHESS_TYPE.LIVE_SIX.value

FIVE = CHESS_TYPE.LIVE_FIVE.value
SFIVE = CHESS_TYPE.CHONG_FIVE.value
...
class ChessAI():
        ...
        for y in range(self.len):
            for x in range(self.len):
                if board[y][x] == 0 and self.hasNeighbor(board, x, y, radius):
                    ...
                    #     fives.append(point)
                    # win state
                    if mscore >= SCORE_SIX or oscore >= SCORE_SIX:
                        sixs.append(point)
                    # 5 in a row
                    elif mscore >= SCORE_FIVE:
                        mfives.append(point)
                    elif oscore >= SCORE_FIVE:
                        ofives.append(point)
                    elif mscore >= SCORE_SFIVE:
                        msfives.append(point)
                    elif oscore >= SCORE_FIVE:
                        osfives.append(point)
                    # 4 in a row
                    # score for four
    ...
    def getPointScore(self, count):
        ...
        if count[FIVE] > 0:
            return SCORE_FIVE
        if count[SFIVE] > 1:
            score += count[SFIVE] * SCORE_SFIVE
        elif count[SFIVE] > 0 and count[FOUR] > 0:
            score += count[SFIVE] * SCORE_SFIVE
        elif count[SFIVE] > 0:
            score += SCORE_FOUR
    ...
    def getScore(self, mine_count, opponent_count):
        ...
        if mine_count[SFIVE] >= 2:
            mine_count[FIVE] += 1
        if opponent_count[SFIVE] >= 2:
            opponent_count[FIVE] += 1
        ...
        if opponent_count[SFIVE] > 0:
            oscore += 1000

        if mine_count[FOUR] > 1:
            mscore += 5000
        elif opponent_count[FOUR] > 0:
            mscore += 1000

        if opponent_count[FOUR] > 1:
            oscore += 20000
        elif opponent_count[FOUR] > 0:
            oscore += 4000

        if mine_count[SFOUR] > 0:
            mscore += mine_count[SFOUR] * 100
        if opponent_count[SFOUR] > 0:
            oscore += opponent_count[SFOUR] * 100
